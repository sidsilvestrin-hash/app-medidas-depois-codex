...
def aplicar_estilo_powerbi():
    st.markdown(
        ...
        <style>
            ...
            [data-testid="stSidebar"] .stCaptionContainer {{ color: #d1d5db; }}
            [data-testid="stSidebar"] [data-baseweb="radio"] label {{
                border-radius: 6px;
                padding: 8px 10px;
                margin-bottom: 4px;
            }}
            [data-testid="stSidebar"] [data-baseweb="radio"] label:hover {{ background: rgba(242, 200, 17, 0.10); }}
            [data-testid="stSidebar"] button {{ border-radius: 6px; border: 1px solid rgba(242, 200, 17, 0.55); }}
            ...
            .antt-title {{ font-size: 1.85rem; font-weight: 800; color: var(--antt-ink); line-height: 1.15; }}
            ...
            .antt-kpi::before {{
                ...
                bottom: 0;
                width: 5px;
                background: var(--kpi-color, var(--antt-accent));
            }}
            .antt-kpi-label {{
            ...
            .antt-section-title {{ color: var(--antt-ink); font-weight: 800; font-size: 1.05rem; margin: 18px 0 8px; }}
            ...
            .stDownloadButton > button:hover {{ border-color: var(--antt-accent); color: var(--antt-accent); }}
...
def page_header(titulo, subtitulo="", chips=None):
    ...
    st.markdown(
        ...
        <div class="antt-page-header">
            <div>
                <div class="antt-eyebrow">Controle ANTT</div>
...
def filtros_medidas(df, prefixo="filtro"):
    if df is None:
        df = pd.DataFrame()
    ...
    if not df.empty and "data_emissao" in df.columns:
        ...
        anos_emissao = sorted(datas.dt.year.unique().astype(int).tolist())
        meses_emissao = sorted(datas.dt.month.unique().astype(int).tolist())
    ano_emissao = c4.multiselect("Ano da emissão", anos_emissao, key=f"{prefixo}_ano_emissao")
    mes_emissao = c5.multiselect("Mês da emissão", meses_emissao, format_func=lambda m: f"{int(m):02d}", key=f"{prefixo}_mes_emissao")
...
def seed_users():
    ...
    if count > 0:
        return
    usuarios = [
        ("admin@antt.gov.br", "admin123", "ANTT", ""),
        ("visualizador@antt.gov.br", "visualizar123", "Visualizador", ""),
        ("litoralsul@concessionaria.com", "litoral123", "Concessionária", "Litoral Sul"),
        ("planaltosul@concessionaria.com", "planalto123", "Concessionária", "Planalto Sul"),
        ("viacosteira@concessionaria.com", "via123", "Concessionária", "ViaCosteira"),
    ]
...
def dashboard():
    ...
    conc, disc, ano_conc, ano_emissao, mes_emissao = filtros_medidas(base_filtros, "dash")
    ...
    with col1:
        ...
        ponto = plotly_bar_navegavel(medidas_tipo, "Tipo", "Quantidade", "Medidas por tipo", "graf_medidas_tipo")
        if ponto:
            tipo = ponto.get("x")
            destino = {"API": NOME_API, "ANC": NOME_ANC, "AE": NOME_AE}.get(tipo)
            prefixo = {"API": "api", "ANC": "anc", "AE": "ae"}.get(tipo)
            if destino and prefixo:
                ir_para(destino, filtros_para_tela(prefixo, conc, disc, ano_conc, ano_emissao, mes_emissao))
    with col2:
        if not df_anc.empty:
            ...
            ponto = plotly_bar_navegavel(sit, "Situação", "Quantidade", "ANCs por situação de prazo", "graf_anc_sit", color="Situação")
            if ponto:
                ir_para(NOME_ANC, filtros_para_tela("anc", conc, disc, ano_conc, ano_emissao, mes_emissao, [ponto.get("x")]))
        else:
    ...
    with col3:
        ...
        if not df_ae.empty:
            ...
            long.append(t)
        if long:
            medida_conc = pd.concat(long, ignore_index=True)
            medida_conc = medida_conc.rename(columns={"concessionaria": "Concessionária"})
            ponto = plotly_bar_navegavel(medida_conc, "Concessionária", "Quantidade", "Medidas por concessionária", "graf_med_conc", color="Tipo", custom_data=["Tipo"])
            if ponto:
                conc_click = ponto.get("x")
                tipo_click = None
                if ponto.get("customdata"):
                    tipo_click = ponto.get("customdata")[0]
                pagina = MAPA_TIPO_PAGINA.get(tipo_click, NOME_ANC)
                prefix = "api" if pagina == NOME_API else "anc" if pagina == NOME_ANC else "ae"
                ir_para(pagina, filtros_para_tela(prefix, [conc_click], disc, ano_conc, ano_emissao, mes_emissao))
        else:
    ...
    with col4:
        if not df_anc.empty:
            ...
            ponto = plotly_bar_navegavel(disc_df, "Disciplina", "Quantidade", "Top disciplinas em ANC", "graf_anc_disc")
            if ponto:
                ir_para(NOME_ANC, filtros_para_tela("anc", conc, [ponto.get("x")], ano_conc, ano_emissao, mes_emissao))
        else:
            st.info("Sem ANCs para exibir.")
...
def tela_cadastrar_medida():
    ...
    tipo = st.selectbox("Selecione o tipo de medida", [NOME_API, NOME_ANC, NOME_AE])
    st.divider()
    if tipo == NOME_API:
        form_cadastro_api()
    elif tipo == NOME_ANC:
...
def form_cadastro_anc():
    ...
    with st.form("form_anc"):
        ...
        disciplina = st.selectbox("Disciplina", DISCIPLINAS)
        data_emissao = st.date_input("Data de Emissão")
        data_recebimento = st.date_input("Data de Recebimento do Ofício")
...
def form_cadastro_ae():
    ...
    with st.form("form_ae"):
        ...
        disciplina = st.selectbox("Disciplina", DISCIPLINAS)
        data_emissao = st.date_input("Data de Emissão")
        data_alinhamento = st.date_input("Data de Alinhamento do Escopo")
        escopo = st.text_area("Escopo da Ação Educativa")
        prazo_cronograma = st.date_input("Prazo para Envio do Cronograma")
        prazo_ae = st.date_input("Prazo da Ação Educativa")
...
def tela_api():
    ...
    st.write("**Data de Emissão:**", item["data_emissao"])
    ...
    if st.session_state["perfil"] == "ANTT":
        ...
        with st.expander("Editar API", expanded=False):
            with st.form("editar_api"):
                ...
                data_emissao = st.date_input("Data de Emissão", value=parse_date(item["data_emissao"]) or date.today(), key="edit_api_data")
...
def tela_anc():
    ...
    if perfil == "Concessionária":
        ...
        with st.form("form_resposta_anc"):
            _, documento = campo_documento_sei("Documento SEI da comprovação", "resp_anc_doc", "", "Somente números")
        ...
        if enviar:
            if not validar_documento_sei(documento):
                st.error("Documento SEI da comprovação deve conter somente números.")
                return
            try:
...
def editar_excluir_anc(anc, id_anc, perfil):
    with st.expander("Editar ANC", expanded=False):
        with st.form("editar_anc"):
            ...
            data_emissao = st.date_input("Data de Emissão", value=parse_date(anc["data_emissao"]) or date.today(), key="edit_anc_data_em")
            data_rec = st.date_input("Data de Recebimento do Ofício", value=parse_date(anc["data_recebimento_oficio"]) or date.today(), key="edit_anc_data_rec")
            ...
            prazo = st.date_input("Prazo Final", value=parse_date(anc["prazo"]) or date.today(), key="edit_anc_prazo")
...
def tela_ae():
    ...
    st.write("**Prazo da AE:**", ae["prazo_acao_educativa"])
    ...
    if perfil == "Concessionária":
        ...
        with st.form("form_acomp_ae"):
            ...
            _, documento = campo_documento_sei("Documento SEI do acompanhamento", "acomp_ae_doc", "", "Somente números")
            observacao = st.text_area("Observação da concessionária")
            arquivo = st.file_uploader("Anexar documento ou evidência (PDF ou HTML)", type=EXTENSOES_UPLOAD)
            enviar = st.form_submit_button("Enviar acompanhamento")
        if enviar:
            if not validar_documento_sei(documento):
                st.error("Documento SEI do acompanhamento deve conter somente números.")
                return
            try:
...
def editar_excluir_ae(ae, id_ae, perfil):
    with st.expander("Editar Ação Educativa", expanded=False):
        with st.form("editar_ae"):
            ...
            data_emissao = st.date_input("Data de Emissão", value=parse_date(ae["data_emissao"]) or date.today(), key="edit_ae_data_em")
            data_alinhamento = st.date_input("Data de Alinhamento do Escopo", value=parse_date(ae["data_alinhamento_escopo"]) or date.today(), key="edit_ae_data_al")
            escopo = st.text_area("Escopo da Ação Educativa", value=ae["escopo"], key="edit_ae_escopo")
            prazo_cron = st.date_input("Prazo para Envio do Cronograma", value=parse_date(ae["prazo_envio_cronograma"]) or date.today(), key="edit_ae_prazo_cron")
            prazo_ae = st.date_input("Prazo da Ação Educativa", value=parse_date(ae["prazo_acao_educativa"]) or date.today(), key="edit_ae_prazo")
...
def main():
    ...
    if "logged" not in st.session_state:
        login_screen()
        return
    st.sidebar.markdown("### Controle ANTT")
    st.sidebar.caption("Medidas preventivas e acompanhamento")
    st.sidebar.markdown(f"**Usuário**  \n{st.session_state['email']}")
    st.sidebar.markdown(f"**Perfil**  \n{st.session_state['perfil']}")
    if st.session_state["perfil"] == "Concessionária":
        st.sidebar.markdown(f"**Concessionária**  \n{st.session_state['concessionaria']}")
    logout_button()
    perfil = st.session_state["perfil"]
    if perfil == "ANTT":
    ...
    pagina = st.sidebar.radio("Navegação", opcoes, key="pagina_atual")
